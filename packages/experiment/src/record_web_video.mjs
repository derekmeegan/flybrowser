...
try {
 ...
 const times=preview?[2000,10000,14000]:Array.from({length:540},(_,i)=>i/30*1000);
 for(let i=0;i<times.length;i++){
  await b.page.evaluate(t=>window.flyVideo.frame(t),times[i]);
  const path=preview?`artifacts/video-preview-${times[i]}.png`:`${directory}/${String(i).padStart(5,'0')}.png`;
